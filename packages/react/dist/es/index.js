...
import { LoggerLevel, WSClient, WSClientEventType, subscribeQuote, extendsFields, snapshotExtendsFields, querySnapshot, QUOTE_CATEGORY_FIELD, queryBrokerQueue, subscribeBrokerQueue } from '@dz-web/quote-client';
...
var uncurryThis$d = NATIVE_BIND$3 && bind$5.bind(call$d, call$d);
...
var ordinaryToPrimitive$1 = function (input, pref) {
  var fn, val;
  if (pref === 'string' && isCallable$f(fn = input.toString) && !isObject$4(val = call$b(fn, input))) return val;
  if (isCallable$f(fn = input.valueOf) && !isObject$4(val = call$b(fn, input))) return val;
  if (pref !== 'string' && isCallable$f(fn = input.toString) && !isObject$4(val = call$b(fn, input))) return val;
...
var QuoteClientContext = /*#__PURE__*/React__default.createContext({
  wsClient: null,
  isWsClientReady: false
...
var QuoteClientProvider = props => {
  ...
  var isConnecting = React.useRef(false);
  ...
  return /*#__PURE__*/React.createElement(QuoteClientContext.Provider, {
    value: {
...
var useQuoteClient = () => {
  var context = React__default.useContext(QuoteClientContext);

  if (!context) {
    throw new Error('could not find quoteClient context value; please ensure the component is wrapped in a <QuoteClientProvider>');
  }
...
var hashCode = (market, code) => "".concat(market, "_").concat(code);

class StockMap {
  constructor(list) {
    _defineProperty(this, "stockMap", void 0);
...
function mergeCommodityDataList(oldData, oldCodeMap, newData) {
  var arr = [...oldData];
  newData.forEach(d => {
    var {
      market: m,
    ...
    var idx = oldCodeMap.getIndex(m, c);

    if (idx !== -1) {
      var rs = _objectSpread2(_objectSpread2({}, arr[idx]), d);
...
function useSubscribeStockListQuote(queryStaticData, dependencies) {
  var {
    wsClient,
    isWsClientReady
  } = useQuoteClient();
  var [stock, setStock] = useState$1([]);
  var stockData = useRef([]);
  useEffect$1(() => {
    var flag = true;
    if (!isWsClientReady || !wsClient || typeof queryStaticData !== 'function') return voidFn;
    var subKey;
    queryStaticData(wsClient).then(function () {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
      if (data.length === 0) return;
      stockData.current = data;
      if (flag) setStock(data);
      var codeList = data.map(i => ({
        market_id: i.marketId,
        code: i.code
      }));
      var stockMap = new StockMap(codeList);
      subKey = subscribeQuote(wsClient, codeList, newData => {
...
function useSubscribeSingleStockQuote(marketId, code) {
  var fields = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
  var hasCodeOrMarketId = code && marketId;
  var rs = useSubscribeStockListQuote(wsClient => hasCodeOrMarketId ? querySnapshot(wsClient, {
    symbols: [[marketId, code]],
    fields: [QUOTE_CATEGORY_FIELD.QUOTE, QUOTE_CATEGORY_FIELD.INFO, ...fields]
  }) : Promise.resolve([]), [marketId, code]);
  var value = rs.length ? rs[0] : {
    marketId,
    code
  };
  return value;
...
var defaultBrokerQueue = {
  buyLevelList: [],
  sellLevelList: []
};
function useSubscribeBrokerQueue(market, code) {
  var {
    wsClient,
    isWsClientReady
  } = useQuoteClient();
  var [data, setData] = useState(defaultBrokerQueue);
  useEffect(() => {
    var flag = true;
    ...
    queryBrokerQueue(wsClient, {
      ...
      setData(res);
      subKey = subscribeBrokerQueue(wsClient, [{
        market_id: market,
        code
      }], newData => {
        var _newData$;

        if (flag) setData((_newData$ = newData[0]) !== null && _newData$ !== void 0 ? _newData$ : {
          buyLevelList: [],
          sellLevelList: []
        });
      });
...
export { QuoteClientProvider, useQuoteClient, useSubscribeBrokerQueue, useSubscribeSingleStockQuote, useSubscribeStockListQuote };
